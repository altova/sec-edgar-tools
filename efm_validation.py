...
def validate_required_facts(instance,error_log,dei_taxonomy,gaap_taxonomy,required_contexts,cikValue,cikNames,submissionType):
    ...
    if not len(required_contexts):
        ...
        if not facts and amendment_flag_value:
            error_log.report(xbrl.Error.create('[EFM.6.5.20] Instance {xbrl} must contain a {qname} fact in the required context {context} when {amendment_flag} was set to {amendment_flag:value}.', severity=xml.ErrorSeverity.WARNING, xbrl=instance.document_element, qname=qname_AmendmentDescription, context=amendment_flag.context, amendment_flag=amendment_flag))
        elif facts and not amendment_flag_value:
            for fact in facts:
                error_log.report(xbrl.Error.create('[EFM.6.5.20] Fact {fact} must not appear in the required context {context} when {amendment_flag} was set to {amendment_flag:value}.', severity=xml.ErrorSeverity.WARNING, location=fact, fact=fact, context=fact.context, amendment_flag=amendment_flag))
        ...
        for rel in network.relationships:
            ...
            if rel.preferred_label:
                if rel.target.period_type == xbrl.taxonomy.PeriodType.DURATION and re_period_start_or_end.search(rel.preferred_label):
                    error_log.report(xbrl.Error.create('[EFM.6.12.7] Presentation arc {arc} with a duration target concept {concept} must not have a {preferredLabel} attribute with role {role}.', severity=xml.ErrorSeverity.WARNING, arc=rel.arc, concept=rel.target, preferredLabel=rel.arc.element.find_attribute('preferredLabel'), role=rel.preferred_label))
