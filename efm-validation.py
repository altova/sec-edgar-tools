...
# This script performs extra checks specified in the EDGAR Filer Manual (Volume II) EDGAR Filing (Version 31) (http://www.sec.gov/info/edgar/edmanuals.htm)
#
...
import os, re, bisect, datetime, imghdr
...
def check_valid_ascii(uri, catalog, error_log):
	...
	with open(uri,catalog=catalog,mode='r',encoding='ascii') as f:
		try:
	...
	if elem.local_name == 'a':
		...
		if src and not re_html_src.fullmatch(src.normalized_value):
			...
			try:
				...
				if imghdr.what(imageuri,open(imageuri,catalog=catalog,mode='rb').read()) not in ('gif','jpeg'):
...
def validate_facts(instance,error_log,catalog,domainItemTypes,textBlockItemTypes,edbody_dtd):
	...
	for fact in instance.facts:
		...
		if isinstance(fact,xbrl.Item):
			...
			# 6.5.12 An instance must not have more than one fact having the same element name, equal contextRef attributes, and if they are present, equal unitRef attributes and xml:lang attributes, respectively.
			...
			if not fact.xsi_nil and fact.concept.type_definition in textBlockItemTypes:
				bContainsQName = False
				for text in fact.normalized_value.split('<')[1:]:
					if re_html_stag.match(text):
						bContainsQName = True
				if bContainsQName:
			...
			if fact.decimals is not None and fact.decimals != float('inf'):
				if fact.numeric_value != fact.effective_numeric_value and ( not fact.numeric_value.is_nan() or not fact.effective_numeric_value.is_nan() ):
					error_log.report(xbrl.Error.create('EFM 6.5.37: Value {fact:value} rounded to {decimals:value} significant figures is {rounded_value} which is not equal to the original value of fact {fact}.', location='decimals:value', fact=fact, rounded_value=str(fact.effective_numeric_value), decimals=fact.element.find_attribute('decimals')))
...
def validate_required_facts(instance,error_log,dei_taxonomy,gaap_taxonomy,required_contexts,cikValue,cikNames,submissionType):
	...
	if not len(required_contexts):
		...
		if not facts:
			error_log.report(xbrl.Error.create('EFM 6.5.20: Instance {xbrl} must contain a {qname} fact in %s.' % required_context_text, xbrl=instance.document_element, qname=qname_AmendmentFlag, context=required_context))
		else:
		...
		if not facts and amendment_flag_value:
			error_log.report(xbrl.Error.create('EFM 6.5.20: Instance {xbrl} must contain a {qname} fact in the required context {context} when {amendment_flag} was set to {amendment_flag:value}.', xbrl=instance.document_element, qname=qname_AmendmentDescription, context=amendment_flag.context, amendment_flag=amendment_flag))
		elif facts and not amendment_flag_value:
			for fact in facts:
				error_log.report(xbrl.Error.create('EFM 6.5.20: Fact {fact} must not appear in the required context {context} when {amendment_flag} was set to {amendment_flag:value}.', location=fact, fact=fact, context=fact.context, amendment_flag=amendment_flag))
		...
		for qname in required_entity_elements.get(document_type_value,[]):
			...
			if not facts or not any(not fact.xsi_nil for fact in facts):
				error_log.report(xbrl.Error.create('EFM 6.5.21: Instance {xbrl} must contain a non-empty {qname} fact in %s.' % required_context_text, xbrl=instance.document_element, qname=qname, context=required_context))
				...
				# 6.5.24 The official Registrant Name that corresponds to the CIK of the xbrli:identifier text content must be a case-insensitive prefix of the dei:EntityRegistrantName fact in the Required Context.
				for fact in facts:
		...
		if document_type_value in ('10-K', '10-Q', '20-F', '10-KT', '10-QT', '40-F'):
			...
			if not len(class_of_stock_facts):
				error_log.report(xbrl.Error.create('EFM 6.5.26: Missing fact {qname} in %s.' % required_context_text, location=instance.document_element, qname=qname_EntityCommonStockSharesOutstanding, context=required_context))
			elif len(class_of_stock_facts) == 1 and None not in class_of_stock_facts:
				for fact in next(iter(class_of_stock_facts.values())):
					error_log.report(xbrl.Error.create('EFM 6.5.26: Fact {fact} in context {context} must be reported without a StatementClassOfStockAxis.', fact=fact, context=fact.context))
			elif len(class_of_stock_facts) > 1 and None in class_of_stock_facts:
				for fact in class_of_stock_facts[None]:
					error_log.report(xbrl.Error.create('EFM 6.5.26: Fact {fact} in context {context} must be reported with a StatementClassOfStockAxis.', fact=fact, context=fact.context))
...
def validate_labels(instance, error_log):
	...
	for label_role in instance.dts.label_link_roles():
		...
		for rel in net.relationships:
			...
			if concept.is_non_numeric() and label.xlink_role in numeric_roles:
...
def validate(catalog, instance, params, error_log):

	CIK = params.get('CIK')
	submissionType = params.get('submissionType')
	cikNames = dict(zip(params['cikList'].split(','),params['cikNameList'].split('|Edgar|'))) if 'cikList' in params else {}
	...
	for taxonomy in instance.dts.taxonomy_schemas:
		if taxonomy.target_namespace:
			if re_dei.match(taxonomy.target_namespace):
				...
				if dei_taxonomy:
					error_log.report(xbrl.Error.create('EFM 6.22: Cannot reference both DEI {dei} and {dei2} taxonomies.', location=next(instance.dts.entry_points), dei=xbrl.Error.Param(dei_taxonomy.target_namespace,location=dei_taxonomy.document.uri), dei2=xbrl.Error.Param(taxonomy.target_namespace,location=taxonomy.document.uri)))
				dei_taxonomy = taxonomy
			elif re_gaap.match(taxonomy.target_namespace):
				# 6.22 Supported Versions of XBRL Standard Taxonomies
				if gaap_taxonomy:
					error_log.report(xbrl.Error.create('EFM 6.22: Cannot reference both US-GAAP {gaap} and {gaap2} taxonomies.', location=instance, gaap=xbrl.Error.Param(gaap_taxonomy.target_namespace,location=gaap_taxonomy.document.uri), gaap2=xbrl.Error.Param(taxonomy.target_namespace,location=taxonomy.document.uri)))
		...
		if taxonomy.document.uri in standard_mapped_uris:
			...
			for concept in taxonomy.concepts:
				standard_concept_names[concept.qname.local_name] = concept
		...
		if schema_location.local_name == 'schemaLocation':
			for uri in schema_location.normalized_value.split()[1::2]:
				if not re_href.fullmatch(uri):
					error_log.report(xbrl.Error.create('EFM 6.3.6: {uri} in attribute {schemaLocation} on {xbrl} must be a standard schema or a relative filename optionally followed by a shorthard xpointer.', location='uri', uri=xml.Error.Param(uri,location=schema_location), schemaLocation=schema_location, xbrl=instance.document_element))
	...
	for doc in instance.dts.documents:
		...
		if not is_extension_document(instance,doc):
			...
			error_log.report(xbrl.Error.create('EFM 6.22: Document {uri} is not a supported XBRL Standard Taxonomy for EDGAR version {version}.', location='uri', uri=doc.uri, children=[hint], version=edgar_version))
			continue
		...
		if isinstance(doc,xbrl.taxonomy.TaxonomySchemaDocument):
			...
			for concept in schema.concepts:
				if isinstance(concept,xbrl.taxonomy.Item):
					...
					concept2 = standard_concept_names.get(concept.qname.local_name,None)
					...
					if recommended_namespace_prefix and concept.id != '{prefix}_{name}'.format(prefix=recommended_namespace_prefix.local_name, name=concept.qname.local_name):				
						name_attr = concept.element.find_attribute('name')
						prefix = xbrl.Error.Param(recommended_namespace_prefix.local_name,location=recommended_namespace_prefix)					
						id_attr = concept.element.find_attribute('id')
						if id_attr is not None:
							error_log.report(xbrl.Error.create('EFM 6.7.17: ID {id:value} of concept {concept} must be the recommended namespace prefix {prefix} followed by one underscore followed by its name {name:value}.', location='id:value', concept=concept, id=id_attr, name=name_attr, prefix=prefix))
						else:
							error_log.report(xbrl.Error.create('EFM 6.7.17: Concept {concept} must have an ID consisting of the recommended namespace prefix {prefix} followed by one underscore followed by its name {name:value}.', location=concept, concept=concept, id=id_attr, name=name_attr, prefix=prefix))
					...
					if concept.qname.local_name.endswith('Axis'):
					...
					if concept.qname.local_name.endswith('Table'):
					...
					if substitutionGroup.qname not in (qname_item,qname_hypercubeItem,qname_dimensionItem):
						error_log.report(xbrl.Error.create('EFM 6.7.25: Substitution group {substitutionGroup:value} of concept {concept} must be either xbrli:item, xbrldt:hypercubeItem or xbrldt:dimensionItem.', location='substitutionGroup:value', concept=concept, substitutionGroup=concept.element.find_attribute('substitutionGroup')))
							
					# 6.7.26 If xsd:element name attribute ends with 'LineItems' then the abstract attribute must equal 'true'.
					if concept.qname.local_name.endswith('LineItems') and not concept.abstract:
						name_attr = concept.element.find_attribute('name')
					...
					if concept.qname.local_name.endswith('Domain') or concept.qname.local_name.endswith('Member'):
	...
	for baseset in instance.dts.base_sets:
		...
		for rel in baseset.relationships:
			if rel.arc.document.uri not in standard_mapped_uris:
				if rel.overriding_relationship is not None:
					...
					error_log.report(xbrl.Error.create('EFM 6.9.3: Relationship {arc} from {source} to {target} is ineffectual because it has been overridden by the relationship {arc2} from {source2} to {target2}.', location=rel.arc, arc=rel.arc, source=source, target=target, arc2=overriding_relationship.arc, source2=source2, target2=target2))
				else:
					...
					if rel.is_prohibited():
						...
						for overridden_rel in overridden_relationships:
							if not overridden_rel.is_prohibited():
								...
								error_log.report(xbrl.Error.create('EFM 6.9.3: Relationship {arc} from {source} to {target} is ineffectual because it overrides the unprohibited relationship {arc2} from {source2} to {target2}.', location=rel.arc, arc=rel.arc, source=source, target=target, arc2=overridden_rel.arc, source2=source2, target2=target2))
		...
		if baseset.arcrole == 'http://www.xbrl.org/2003/arcrole/summation-item':
			...
			for rel in network.relationships:
				...
				# 6.16.8 The target of an effective relationship with an xlink:arcrole attribute equal to 'http://xbrl.org/int/dim/arcrole/notAll' must not be the target of an effective arc with an xlink:arcrole attribute equal to 'http://xbrl.org/int/dim/arcrole/all' in link:definitionLink elements having equal values of xlink:role.
				...
				if len(all_relationships):
					error_log.report(xbrl.Error.create('EFM 6.16.8: Hypercube {hypercube} must not be a target of all relationship {all} and notAll relationship {notAll} within the same link role {role}.', location=rel.arc, notAll=rel.arc, all=all_relationships[0].arc, hypercube=rel.target, role=baseset.role))
	...
	for doc in instance.dts.documents:
		...
		for schema_location in doc.schema_location_attributes:
			if schema_location.local_name == 'schemaLocation':
				for uri in schema_location.normalized_value.split()[1::2]:
					if not re_href.fullmatch(uri):
						error_log.report(xbrl.Error.create('EFM 6.3.6: {uri} in attribute {schemaLocation} on {elem} must be a standard schema or a relative filename optionally followed by a shorthard xpointer.', location='uri', uri=xml.Error.Param(uri,location=schema_location), schemaLocation=schema_location, elem=doc.document_element))
		...
		if isinstance(doc, xbrl.taxonomy.LinkbaseDocument):
			...
			for arcrole_ref in linkbase.arcrole_refs:
				...
				if arcrole_ref.xlink_href.partition('#')[0] not in standard_uris:
					error_log.report(xbrl.Error.create('EFM 6.9.6: Arcrole URI {arcroleURI:value} on {arcroleRef} must be defined in the XBRL 2.1 specification or a standard taxonomy schema.', location='arcroleURI:value', arcroleRef=arcrole_ref.element, arcroleURI=arcrole_ref.element.find_attribute('arcroleURI')))
			...
			for link in linkbase.extended_links:
				...
				for arc in link.arcs:
					...
					if arc.priority >= 10:
						...
						error_log.report(xbrl.Error.create('EFM 6.9.9: Priority {priority:value} on arc {arc} must be less than 10.', location='priority:value', priority=priority_attr, arc=arc.element))
					...
					for arc in link.arcs:
						for rel in arc.relationships:							
							...
							if rel.source.document.uri in standard_mapped_uris and rel.target.xlink_role == 'http://www.xbrl.org/2003/role/documentation':
								if rel.is_prohibited():
									error_log.report(xbrl.Error.create('EFM 6.10.5: Label {label} must not be removed from standard concept {concept}.', location=arc.element, label=resource, concept=rel.source))
								else:
									error_log.report(xbrl.Error.create('EFM 6.10.5: Label {label} must not be added to standard concept {concept}.', location=arc.element, label=resource, concept=rel.source))
				...
				if link.qname == qname_referenceLink:
					...
					for arc in link.arcs:
						...
						if order is None or not order.specified:
							error_log.report(xbrl.Error.create('EFM 6.12.1: Presentation arc {arc} must have an order attribute.', location=arc.element, arc=arc.element))
					...
					for arc in link.arcs:
						...
						order = arc.element.find_attribute('order')
						if order is None or not order.specified:
							error_log.report(xbrl.Error.create('EFM 6.14.1: Calculation arc {arc} must have an order attribute.', location=arc.element, arc=arc.element))

						# 6.14.2 Element link:calculationArc requires a weight attribute value equal to 1 or -1.
						if abs(arc.weight) != 1:
							error_log.report(xbrl.Error.create('EFM 6.14.2: Calculation arc {arc} must have a weight attribute equal to 1 or -1.', location='weight:value', arc=arc.element, weight=arc.element.find_attribute('weight')))
					...
					for arc in link.arcs:
						...
						order = arc.element.find_attribute('order')
						if order is None or not order.specified:
							error_log.report(xbrl.Error.create('EFM 6.16.1: Definition arc {arc} must have an order attribute.', location=arc.element, arc=arc.element))

	presentation_networks = []
	for presentation_role in instance.dts.presentation_link_roles():
		network = instance.dts.presentation_base_set(presentation_role).network_of_relationships()
		...
		for rel in network.relationships:
			if (rel.source,rel.order) in source_to_relationship:
				...
				if rel.arc.document.uri not in standard_mapped_uris or rel2.arc.document.uri not in standard_mapped_uris:
					error_log.report(xbrl.Error.create('EFM 6.12.2: Presentation arcs {arc} and {arc2} within the same base set starting from same source element {source} must have distinct values of the order attribute.', location=rel.arc.element, arc=rel.arc.element, arc2=rel2.arc.element, source=rel.source))
			else:
	...
	for calculation_role in instance.dts.calculation_link_roles():
		...
		for rel in network.relationships:
			...
			if rel.source.period_type != rel.target.period_type:
				error_log.report(xbrl.Error.create('EFM 6.14.3: The source {source} and target {target} of relationship {arc} must have equal values of xbrli:periodType attribute.', location=rel.arc.element, arc=rel.arc.element, source=rel.source, target=rel.target))
			...
			if used_concepts.get(rel.source,False) and used_concepts.get(rel.target,False) and not has_concepts_in_presentation_linkbase(instance.dts,rel.source,rel.target):
				error_log.report(xbrl.Error.create('EFM 6.14.5: The source {source} and target {target} of calculation relationship {arc} must also have effective presentation relationships with the same extended link role.', location=rel.arc.element, arc=rel.arc.element, source=rel.source, target=rel.target))
	...
	for concept in used_concepts.keys():
		...
		for rel in network.relationships_to(concept):
			if (rel.source,rel.preferred_label) in source_to_relationship:
				...
				error_log.report(xbrl.Error.create('EFM 6.12.5: Presentation arcs {arc} and {arc2} in the same base set with the same source and target must have distinct values of the preferredLabel attribute.', location=rel.arc.element, arc=rel.arc.element, arc2=rel2.arc.element))
			else:
...
def on_xbrl_finished_dts(job, dts):
	if dts is not None:
		...
		if job.script_params.get('forceUtrValidation', False):
			bEnableUTR = True
		else:
...
def on_xbrl_finished(job, instance):
	...
	if instance is not None:
		validate(job.catalog, instance, job.script_params, job.error_log)
	else:
		# 6.4.3 The XBRL instance documents in a submission must be XBRL 2.1 valid.
		xbrl_errors = list(job.error_log.errors)
		job.error_log.clear()
		job.error_log.report(xbrl.Error.create('EFM 6.4.3: Instance {uri} is not a valid XBRL 2.1 document.', location=job.input_filenames[0], children=xbrl_errors, uri=job.input_filenames[0]))
