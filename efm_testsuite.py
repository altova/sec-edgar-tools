...
re_collapse = re.compile(r'[ \t\n\r]+')

utr = xbrl.UnitsRegistry.default_utr()
xhtml_inlinexbrl_xsd, _ = xsd.Schema.create_from_url('http://www.xbrl.org/2013/inlineXBRL/xhtml-inlinexbrl-1_1.xsd')
...
def execute_variation(testcase, variation):
    ...
    if os.path.splitext(uri)[1] == '.htm':
        ...
        if bNotEDGARDependent:
            instances, error_log = xbrl.InlineXBRLDocumentSet.transform_xml_from_url(uri)
        else:
            ...
            if not error_log.has_errors():
                instances, error_log = xbrl.InlineXBRLDocumentSet.transform_xbrl_from_url(uri, utr=utr if bEnableUTR else None)
...
def write_xml_report(path, testsuite, results, runtime, relative_uris):
    ...
    with open(path, 'w') as xmlfile:
        ...
        xmlfile.write('\turi="{}"\n\tname="{}"\n\ttotal="{}"\n\tfailed="{}"\n\tskipped="{}"\n\tconformance="{}"\n\truntime="{}"\n\texecution-date="{:%Y-%m-%dT%H:%M:%S}"\n\tprocessor="Altova RaptorXML+XBRL Server">\n'.format(testsuite_uri,
                                                                                                                                                                                                                                testsuite['name'], total, failed, skipped, conformance, runtime, datetime.datetime.now()))
        for testcase in testsuite['testcases']:
